...
--
Describe the architecture of the program in the Ghidra project using aipiler_get_entry_function_name, aipiler_list_functions, and aipiler_read_function_code, save it to ./ARCHITECTURE.md , it must be really precise and full of diagrams, with reviewed functions being specified and documented in ./FUNCTION_SIGNATURES.md (Read the specification in the file before writing.), also write each function signature to ./funcs/FUNCTION_NAME.signature. If it helps, create a ./QUESTIONS.md file to add and answer questions related to the architecture.
...
subprocess.run(["opencode", "run", "-m", get_model(), prompt], cwd=documentation_folder)
